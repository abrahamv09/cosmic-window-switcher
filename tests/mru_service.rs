...
use cosmic_window_switcher::{
    CaptureBackend, CaptureBackendSelection, CaptureBackendState, DmaBufFallbackReason, Locale,
...
fn status_reports_the_active_capture_backend_and_privacy_safe_fallback_reason() {
    ...
    assert_eq!(
        diagnostics
            .capture_backend
            .selection()
            .expect("capture negotiation completed")
            .backend(),
    ...
    assert!(status.contains("capture_backend_fallback: DMA-BUF renderer import is unavailable"));
